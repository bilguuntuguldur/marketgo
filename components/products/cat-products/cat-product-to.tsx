...
const CatProductTwo: React.FC<ProductProps> = ({ categoryId, post_per_product }) => {
    ...
    useEffect(() => {
        const initializeCarousel = async () => {
            if (typeof window !== "undefined" && !loading && products.length) {
                ...
                const pagination = parseInt($tabcateSlideConf.attr("data-pagination") || "0");
                const pausehover = parseInt($tabcateSlideConf.attr("data-pausehover") || "0");
                ...
                jQuery("ul.tab_cates li").click(function () {
                    ...
                    jQuery("#" + activeTab).fadeIn().addClass("animatetab");
                });
            }
        };
    ...
    if (error) {
        return <div className="conten-box">Error: {error}</div>;
    }
    ...
    return (
        <div id="tab_23" className="tab_category">
