...
const ListProductCard: React.FC<ProductCardProps> = ({ product, isAboveTheFold = false }) => {
    const cart = useCart();
    ...
    return (
        <div className="item_manu">
            <article className="product-miniature js-product-miniature" data-id-product={product.id}>
                <div className="img_block">
                    <Link href={`/products/${product.slug}`} className="thumbnail product-thumbnail">
                        <Image
                ...
                <div className="product_desc">
                    <div className="manufacturer"><Link href={`/products/${product.slug}`}>{product.categories[1]?.name}</Link></div>
                    <h1 itemProp="name"><Link href={`/products/${product.slug}`} className="product_name">{product.name}</Link></h1>
